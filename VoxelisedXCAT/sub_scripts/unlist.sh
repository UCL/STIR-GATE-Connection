...
if [ $# -ne 2 ]; then
  ...
  exit 1
	...
	echo "Unlisting $StoreRootFilesDirectory/ROOT_OUTPUT_$SGE_TASK_ID.root"
fi
...
sed -i.bak "s/SGE_TASK_ID/$SGE_TASK_ID/g" lm_to_projdata_$SGE_TASK_ID.par
sed -i.bak "s/UNLISTINGDIRECTORY/UnlistedSinograms/g" lm_to_projdata_${SGE_TASK_ID}.par


cp  Templates/root_header_template.hroot  root_header_${SGE_TASK_ID}.hroot
sed -i.bak "s/SGE_TASK_ID/${SGE_TASK_ID}/g" root_header_${SGE_TASK_ID}.hroot
sed -i.bak "s/LOWTHRES/${LowerEnergyThreshold}/g" root_header_${SGE_TASK_ID}.hroot
sed -i.bak "s/UPTHRES/${UpperEngeryThreshold}/g" root_header_${SGE_TASK_ID}.hroot
