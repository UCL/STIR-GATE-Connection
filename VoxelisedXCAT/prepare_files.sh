...
if [ $SCANNERTYPE = "D690" ]; then
	...
	cp -v $D690_DIR/csorter_D690.mac csorter.mac
	cp -v $D690_DIR/digitiser_D690.mac digitiser.mac
	cp -v $D690_DIR/geometry_D690.mac geometry.mac

elif [ $SCANNERTYPE = "mMR" ]; then
	echo "Preparing mMR scanner files"
	mMR_DIR="../ExamplesOfScannersMacros/mMR"
	cp -v $mMR_DIR/csorter_mMR.mac csorter.mac
	cp -v $mMR_DIR/digitiser_mMR.mac digitiser.mac
	cp -v $mMR_DIR/geometry_mMR.mac geometry.mac

else
	echo "Invalid scanner selection parsed. Please indicate 'D690' or 'mMR'."
fi
